...
def main():
    ...
    for gidx, group_key in enumerate(sorted(groups.keys())):
        print(f"Plotting group: {group_key}")
        gtol_dict = groups[group_key]
        ...
        for i, gtol in enumerate(sorted_gtols):
            ...
            if i < len(sorted_gtols) - 1:
                ...
                label = f"{group_key}"
                plt.semilogy(loss_trace, label=label, linestyle=linestyles[gidx])
    ...
    pdf_path = ASSETS_DIR / "loss_traces.pdf"
